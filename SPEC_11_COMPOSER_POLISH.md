...
- **フォルダ書き出しの実ダイアログだけは自動検証できていない**（`showDirectoryPicker` はユーザー操作が必須で、
  OSのフォルダ選択ダイアログはブラウザ制御の外）。関数の存在・`isSecureContext`・実クリックで呼び出しに入ること・
  モックしたディレクトリハンドルへ 10枚（`frame_00011`〜`frame_00020`）書けること・例外時のZIPフォールバックは確認済み。
  **初回だけ実機で「押してダイアログが出るか」を確認すること。**

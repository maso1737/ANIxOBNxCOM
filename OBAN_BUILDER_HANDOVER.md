...
## 未着手・次チャット候補（2026-08-05 発注者フィードバック・UI微調整）

1. **animator/composerとのUI仕様統一**: フローティングパネルの閉じ方を✕ボタンに統一（現状はESC等の別方式）。
   FXボタンはON時に光っている状態（`.primary`等の点灯表現）にする。
2. **GUIDEボタンを廃止し、その位置にCLEARボタンを配置**。
3. **メニュー項目の表記を「▶ ANIMATOR」「▶ COMPOSER」に変更**（矢印付き表記へ統一）。
4. **メニュー下の使い方解説を、SETTINGSのショートカット一覧の上部に移設**。

いずれも見た目・配置の微調整で機能追加はなし。着手時はanimator.html/composer.htmlの該当UIパターンを先に確認して合わせること。

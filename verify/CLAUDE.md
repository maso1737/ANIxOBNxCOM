...
3. **撮る面が2つある。`seek(t)` の t で撮り分ける**（契約は「t→状態」なので合法）。
   ...
   | `>= 10000` | `cells` | `drawCellPlate` を 640×360 の升目 2×2 に **紙の縦横比のまま**。**GRID セル＝その紙（プレート）の全面** |

   ★ **この2つを両方撮るのが econte 版の肝。** V5（SPEC_19）で
   「GRIDセル＝紙の全面／出力＝そのコマで画を持っている紙だけ（homePlate）」と**意図的に分けてある**ので、
   片方だけ見ていても T.U./PAN の食い違い（差し替えの時刻・紙の縁）に気づけない。

4. **フィクスチャ（`ehBuildFixture`）はカット7本 / 全92コマ / 24fps。**（C6/C7 は 2026-09-23・V5 で追加）

   | | 構成 | 何を踏むか |
   |---|---|---|
   | C1 | FIX（枠1つ・12コマ） | いちばん素直な経路。line と plate（0.4倍解像度）の両方 |
   | C2 | **T.U**（A=全景 → B=寄り・lead4/move12/tail4） | **紙が2枚**。A の紙だけの線／B の紙だけの線と色／紙に引く線（両方）。寄りの紙へ差し替わる時刻（homePlate） |
   | C3 | **PAN**（A→B が斜めに並ぶ・10コマ） | **紙1枚**＝境目そのものが無い（途中のコマに縁が出ない） |
   | C4 | 投げ縄（8コマ） | `lassoFillPolygon`＝`strokeSeg` とは別経路（スキャンライン）＋消し（`destination-out`） |
   | C5 | 空カット（6コマ） | 紙も加筆も無い＝bg だけ出る経路 |
   | C6 | **縦 PAN**（A=上 → B=下・10コマ） | 縦長の紙1枚。GRID では縦長のセル |
   | C7 | **PAN → T.U.**（A→B 横 PAN、B の中の C へ寄る・lead2/8/12/tail4＝26コマ） | 紙2枚（横長 [A,B] と寄り [C]）。到着のコマで寄りの紙へ差し替わる |

   絵は **`ehStroke()` が econte 自身の `strokeSeg()` を呼んで**引く（ブラシ幅・色・ツール・塗り先がそのまま回帰対象）。
   `opt.pl` で行き先の紙を1枚に決める（画面で描くのと同じ経路）。付けなければ「紙に引く」＝重なる紙全部へ。
   ★ V4 のベースラインの C1 の色（brush 40）は **2.5倍太かった**（旧 `gStrokeK` が初期値 1 のまま残っていた＝ハーネスだけの誤り）。
     V5 は紙ごとに `plK` を引くので出力 40px が正しい値。

### ★ `ehPatchMarks()` が要る理由 —— 負のコントロールが自己相殺した話（2026-09-03）

最初、パッチの中身を `ehStroke()` だけで作っていた。この状態で
**`camBakeRect`（V5 の `plBakeRect`）の x を +2px ずらす負のコントロールを当てても、9コマ中8コマが 0.000% のまま通ってしまった。**

原因は対称性だった。`strokeSeg` → `strokePatchSegBake` → **`bakeToPatch()` も `camBakeRect` を使う**（V5 では `bakeToPl` / `plBakeRect`）。
フィクスチャは毎回その場で塗り直すので、
...
そこで `ehPatchMarks()` で、**座標マッピングを一切通さない絵**（紙の縁・四隅の目印・逆対角線を
プレートのピクセル空間へ直接）を全カットの全プレートに入れてある。**この関数を消すと検証の芯が抜ける。**

> 一般化: **フィクスチャを「アプリの関数で組み立てて、同じ関数で描く」と対称性で穴が空く。**
> 他ツールへ広げるときも、必ず片側だけを通る素材を混ぜること。

### 予算（ECONTE・2026-09-23 実測・V5）

| 項目 | 実測 | 上限 | 何を捕まえるか |
|---|---|---|---|
| `maxTTFFms` | 202〜224ms | 800 | 起動＋紙6枚の生成＋フィクスチャのペイント |
| `maxLongestFrameMs` | 173〜192ms | 300 | 最長フレーム（起動時のスクリプト評価が主） |
| `maxVramBytes` | 78.3MB | 112MB | 紙6枚（各3.7MB）＋プレート（PAN の紙は外接矩形ぶん大きい：C3 2560×1440 など） |
| `allowedPixelRatio` | 0.000% | 0.02% | 他3ツールと同じ |

（V4 時代: 39.2MB / 上限 64MB。C6/C7 の追加と、斜め PAN の紙が1枚の外接矩形になったぶん増えた）

### 負のコントロール（V5・2026-09-23 実測・15コマ）

| 壊し方 | 結果 | 通ったコマの説明 |
|---|---|---|
| 紙の置き場所 `plBakeRect` の x を **+2px**（筆の側） | **14/15 FAIL**（0.033〜2.225%） | `e06_c4_lasso` 0.003%（投げ縄は座標変換を通らない）／`e07_c5_empty` 0%。**どちらも正しい** |
| 出力の置き場所 `plDest` の x を **+2px**（合成の側） | **14/15 FAIL**（0.113〜1.082%） | `e07_c5_empty` だけ 0%。**紙が1枚も無いカット**なので正しい |

＝ 置き場所（合成側）と 筆（ペイント側）が**別々に**見張られていることの確認。
（V4 時代の記録: `camBakeRect` +2px → 8/9 FAIL ／ ブラシ幅 5% 細く → 7/9 FAIL）

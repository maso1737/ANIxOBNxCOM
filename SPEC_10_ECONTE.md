...
- **V2-E2（実装済み・2026-08-02）**: カラースクリプト編集（EDITの`GRID`モード＋カラー層＋描画モード5種）。
  E2b=GRIDの作り直し（黒背景・拡縮・行カラー帯・尺バー）／E2c=カラー層を枠ごとへ＋複数選択の一括変更／
  E2d=ANIMATOR準拠のブラシ操作（SPEC_13 §9b・スキル `animator-brush-ops`）
- **V2-D3（実装済み・2026-08-09）**: INFO帯（焼き込み3モード）＋MP4書き出し（WebCodecs・非実時間）＋
  カラースクリプト一覧PNG（SPEC_13 §5f/§5g/§5h）
...
- ショートカット（`econte_keymap_v1`・kit準拠レジストリ。**V2-A以降は STUDIO ビューで有効**）:
  ...
  **V2-A で Space がトランスポートに移ったため、BOARDの強制パンは Alt / 中ボタン**
  （空白ドラッグ＝パンは従来どおり）。
- **EXPORT VIDEO**: V2-D3 で **MP4（WebCodecs・非実時間）** に置換。詳細は「出口」。
  書き出し中はSpace/中断で止められる（ファイル破棄）。
  **フォールバックの実時間録画のときだけ**、従来どおりタブを前面のままにする必要がある
  （非表示タブはrAF停止のため録画が止まる）。WebCodecs経路にこの制約は無い。
...
- **カラースクリプト層 `cut.colors[k]`（V2-E2c）**: **枠(cam[k])ごとに1枚**・512×288・
  **出力枠1280×720の空間**・**LRU対象外で常駐**。枠ごとに `blend`（乗算/通常/加算/発光/オーバーレイ）＋
  `alpha`。保存は PNG。**T.U.のA枠とB枠は独立して塗れる**（ベイク空間だと必ず同期してしまう）。
...
- ツール: **PEN / ERASE / FILL**（合成色を境界判定して drawC に書く）。**EYEツールは無い**
  （Alt+クリックでスポイト）。手振れ補正なし。ブラシサイズは**出力1280px基準**（線画1–64／色1–300）。
- **操作は ANIMATOR 準拠**（スキル `animator-brush-ops`）: 右ドラッグでサイズ／PEN Wクリックで筆圧／
  **FILL Wクリックで透明消し**／FILL長押しでバケツ⇔投げ縄塗り／Tabで TOOLパネル左右入替。
  ERASE WクリックでCLEAR は econte 独自の追加。
...
- **動画コンテ MP4（V2-D3実装済み）**: `WebCodecs(H.264)+mp4-muxer` で**非実時間**書き出し。
  未対応環境は `MediaRecorder(mp4→webm)` の実時間録画へ自動フォールバック。
  焼き込みは **OFF / 画面内 / INFO帯** の3モード（`renderOutputFrame()` がプレビューと共通＝WYSIWYG）。
  INFO帯は絵を1/1.2に縮めて中央に置き、余白の帯に `C#／尺／note先頭行`（画に重ならない）。
- **カラースクリプト一覧PNG（V2-D3実装済み）**: 枠単位・4列・セル480×270。
  尺バー／キャプション（`C1-A ／ A→B 60Fr`）／代表色の帯つき。**GRIDの見た目をそのまま大きく描く**。

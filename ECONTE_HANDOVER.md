...
- **V3-P2「浮いた選択」＝軽いフォトバッシュ: 実装済み（2026-08-19・SPEC_15 P2）**

  Photoshop で投げ縄→ドラッグしたときの手触り。**同時に1枚だけ・保存しない**ので
  **LRU も保存フォーマットも一切触っていない**（旧 V2-E3 の `cut.layers[]` を採らなかった理由がここ）。

  | | 変えたこと | 要点（次に触る人向け） |
  |---|---|---|
  | 1 | `SEL` ツール（`A`）を追加 | PEN/ERASE/LASSO の隣。**SINGLE 以外ではグレー**（`syncSelToolUI`）。GRIDのセル・STUDIOプレビューで使おうとするとトーストで断る |
  | 2 | 中身は **紙（ベイク空間）の1枚**として持ち上げる | `floatSourcePaper()` が `drawPatchStack(…, raw)` で層をベイク寸法に合成 → 多角形で `destination-in` マスク。**`raw` は plate のときだけ効き、濃/モードを掛けない**（同じ層へ戻すので往復で色が濃くならない） |
  | 3 | 確定は **重なる枠ぶん全部のパッチへ落とす** | `floatEachFrame()`。**投げ縄塗り（`lassoEnd`）とまったく同じ配り方**にした＝面で効く操作の振る舞いがツール間でそろう。切り取りも同じ枠集合から抜くので、同じ場所で確定すれば往復で元に戻る |
  | 4 | **★焼き込み先はつねに「いまの描く先」**（`floatKindNow()`） | SPEC_15 P2-4 は「色も line へ焼く」だったが、それは plate がカット1枚だった V3 の話。V4 で plate も枠ごとのパッチになった以上、color を line に落とすと**色帯・C.SCRIPT から絵が消える**。**持ち上げ元＝Shiftなら REF・それ以外は描く先／焼き込み先＝描く先** の1行に統一した（2026-08-19 の追い実装）。おかげで「color のとき Shift で REF を切り貼り」も「line で持ち上げて color に落とす」も同じ規則で通る。行き先は浮いた枠にラベルで出す |
  | 5 | Undo は「持ち上げ＋確定」で**1手** | 持ち上げで `txBegin()` → 切り取りを `txTouch` → **`gTx` を `gFloat.tx` へ退避して `gTx=null`**（浮いている間に他が触っても壊されない）→ 確定で書き戻して `txEnd()`。実測で Ctrl+Z 1回が持ち上げ前と**完全一致** |
  | 6 | `Esc` は **ログを汚さない** | `txAbort()` が控えを `globalCompositeOperation='copy'` で丸ごと戻して捨てる。控えはパッチ1枚ぶんなので、**浮いている間はペイントを塞いである**ことが前提（`paintDown` が先に float 経路へ流す） |
  | 7 | 操作の割り当て | ドラッグ＝切り取り／`Ctrl`＝コピー／`Shift`＝REFからコピー。**`Alt` は既にスポイトなので使えない**（SPEC_15 P2-3 の「Altドラッグ＝コピー」はここだけ移した） |
  | 8 | 変形枠は**画面px一定** | `floatScreenK() = gEditFit * editView.z` で割る。ズームしても枠線・ハンドルの太さが変わらない |
  | 9 | 浮いている絵の描き場所 | **`renderEditCanvas()` の中だけ**。`compositeTo()` には入れない＝サムネ・GRID・書き出しには**確定するまで存在しない**（実測: `drawCamFrame`/`drawCellFrame` で該当領域 0px・editCv だけ 7608px） |
  | 10 | 抜け道を塞いだ | ツール変更・`setEditMode`・`setView`・`openEdit`（**curCut を動かす前**）で確定。**SEL を持ったまま SINGLE を出たら PEN に戻す**（グレーのボタンと選択中のツールが食い違うと「なぞっても何も起きない」が仕様に見えない） |
  | 11 | 断る条件 | bbox が `MAX_AREA/2` 超え＝「範囲が大きすぎます」／中身が透明だけ＝「選んだ範囲に画がありません」（`canvasHasInk` は 64px に縮めてから見るので大きい範囲でも軽い） |

  **安全弁（SPEC_15 P2-4）は動かしていない**: **BASE（REF）からはコピーしか取れない**。
  元に穴を開けないので ⟳（BOARDから再ベイク）がいつでも安全に押せる。
  実測でも持ち上げ・確定の前後で `baseC` の画素数が1つも変わらないことを確認済み。

  **失っているもの**: 確定後の変形やり直し（Ctrl+Z で戻してやり直す）。

- **V3-P2b「変形」: 実装済み（2026-08-19・SPEC_15 P2-7）** — 回転／自由変形／WARP を足した。

  | | 変えたこと | 要点（次に触る人向け） |
  |---|---|---|
  | 1 | **★形は `箱(x,y,w,h) + rot + warp` で持つ。`warp` の制御点は箱ローカルの正規化座標 `{u,v}`** | ここが全部の土台。**ワールド座標で持つと、箱を動かす/回す/拡縮するたびに制御点を作り直すことになる**。ローカルなら「箱を動かせば変形した絵も付いてくる」が自動で成り立つ |
  | 2 | 貼り方は3段 | 変形なし＝**アフィン1回**（`ctx.transform`・最速経路）／`Ctrl`+隅＝**射影変換**（`flHomo`。bilinear だと台形が台形に見えない）／WARP＝4×4の**Catmull-Rom**（`flCr1`。端は接線を外挿して4点でも素直に通す） |
  | 3 | メッシュの継ぎ目 | `flDrawTri`（3点アフィン＋clip）を敷き詰め、**重心から 0.45px 膨らませて**隠す。べた塗りを大きく回転＋WARPさせて内側の抜けが **0px** であることを実測 |
  | 4 | 回転は**隅の外側のリング**（`FL_ROT_PX`＝42画面px） | PSと同じ。ホバーで **↻ カーソル**（SVGのdata URI）を出すので説明が要らない。それより外は従来どおり「クリック＝確定」。ペンで掴むので 30 → 42px に広げた（30 だと帯が20pxしかなく細かった） |
  | 5 | **`Shift` は「縦横比維持」**（当初と逆） | 既定を自由にした。PSの手癖に合わせる |
  | 6 | 拡縮は**反対側の隅をワールドで固定** | warp が入っていてもアンカー／手の**ローカル位置**を通して箱の w/h を解く（`d.du`/`d.dv`）。実測で回転52.7°付きでもアンカーが 0.00px も動かない。ここを素直に「x,y,w,h を書き換える」だけにすると、**回転の中心が動いて絵が飛ぶ** |
  | 7 | WARP は SEL の**Wクリック** | LASSO/L-DEL と同じ手つき（SPEC_16 §5-B3）。**OFF にしても形はそのまま残る**（点が消えるだけ）＝作業が飛ばない。ON/OFF は `UI_LS` |
  | 8 | 重さ | **メッシュ化しても増えない**。ベイク1813×1020／1450×802 の大物で 変形なし 3.13ms ／ 自由変形 3.39ms ／ WARP 3.23ms（紙の合成のほうが支配的）。確定は最大 173ms の1回きり。**「ワープは重いなら要らない」は当たらなかった** |

  **入れていないもの**: メッシュの再編集（確定したら形は焼ける）／WARPの分割数変更（3×3固定）。

- **GRID / SINGLE の並びとキーを入れ替えた（2026-08-19）** — ボタンは **左=GRID / 右=SINGLE**、
  数字キーも **3=GRID / 4=SINGLE**。理由は「EDITは GRID から入って、1枚を詰めに SINGLE へ行く」という
  実際の手順と、左から右の並びを合わせるため。`SHORTCUT_ACTIONS` の**配列の順**も入れ替えたので
  ⚙ の一覧も同じ並びで出る。**キーマップはアプリが localStorage へ書かない**（再割当は
  `econte_keymap_v1` を手で編集する運用）ので、`def` を変えるだけで全員に効く。

- **GRID と SINGLE でペイントの解像度は変わらない（2026-08-19・ユーザー質問に実測で回答）**
  どちらも **同じパッチ**（`line` 1280×720 ／ `plate` 512×288）へ直接書く。GRIDのセルは
  `cellPatchCoord()` が `getBoundingClientRect()`（CSSpx・連続値）からパッチ座標へ直接飛ばすので、
  **セルキャンバスの内部解像度（`GC_MIN_RES` 320〜`GC_MAX_RES` 1280）は表示専用**で
  書き込み先には関係しない。ブラシ幅も `patchK()` が line=1 なので同じ。
  実測: 同じ論理座標(300,200)→(900,500) の線を SINGLE と GRID で引いて
  **ink 6067 / bbox [295,195,904,504] が完全一致**。
  違うのは **手の精度**（セルが 200CSSpx なら 1画面px ≒ 6.4パッチpx）と**プレビューの粗さ**だけ。
  詰めたいところは SINGLE で、というのは「精度」の話であって「解像度」の話ではない。

- **§5-C5 画ブレ / §5-C6 イーズ（AE準拠・キーごと）: 実装済み（2026-08-19）**

  ### C5 画ブレ — **キーは打たない**
  当初の案（MOVE で PAN と同じようにセットしてキーを打つ）は取らなかった。
  実際に欲しいのは「**強めていく**」「**弱めてブレがなくなる**」の2つだけだったので、
  カットに1組だけ持たせる。

  | | 仕組み | 要点 |
  |---|---|---|
  | 1 | `cut.shake = { a0, a1, freq }` | a0=頭の強さ / a1=尻の強さ（0〜100）を**直線で補間**。一定=同じ値 / 強めていく=0→強 / 弱めてなくなる=強→0 |
  | 2 | 揺れ幅は **枠の幅に対する割合** | `SHAKE_MAX = 0.045`（100%で枠の4.5%）。T.U.で寄っても画面上の揺れ量が変わらない |
  | 3 | **乱数を持たない** | `shakeSeed(cut)`（idのハッシュ）＋値ノイズ。**コマ番号から決まる**ので再生し直しても書き出しても同じ揺れ。保存も不要 |
  | 4 | 乗せるのは **出力だけ** | `drawCamFrame`（プレビュー/録画/書き出し）のみ。`drawCellFrame`（GRIDセル / SHEETサムネ / C.SCRIPT / 控え）は**揺らさない**（一覧が動くと選べない） |
  | 5 | FIXカットも効く | `camSrcRect` は枠列カットしか切り出しを返さないので、FIX 用に `fixShakeRect()` を別に置いてある |
  | 6 | 中央プレビューで塗る座標もブレ込み | `toBakeCoord(tlCv)` → `camToBake(..., true)`。**揺れはコマ番号から決まる**ので、止めて描いている限りズレようがない |
  | 7 | UI | BOARD で枠を選んだときのバー（＋枠 の隣）の **∿**。OFFは記号だけ / ONは `∿ 0→70`。中身は頭/尻/速さの3スライダー＋プリセット4つ |
  | 8 | SHEET | `rowFlags` に **∿**（使っているカットが一覧で分かる） |
  | 9 | Undo | スライダーを引いている間は `quiet` で積まず、`change` で1手（`cut-shake`） |

  **入れていないもの**: 回転ブレ（`camSrcRect` は軸揃えの矩形しか扱わないので、
  入れるなら合成側に回転描画を足す必要がある）。現状は平行移動のみ。

  ### C6 イーズ — **キーごとの効き（AE / COMPOSER と同じ持ち方）**
  旧実装は「**区間に1つの名前**」（LINEAR/IN/OUT/Q.PAN/…）だったので、
  どちらのキーの話なのかが読めない。しかも **旧 `IN`=遅い出だし は AE では Ease Out** で名前が逆だった。

  | | 変えたこと | 要点 |
  |---|---|---|
  | 1 | `cam[k].ease`（区間）→ **`cam[k].eo` / `cam[k].ei` / `cam[k].hold`**（キー） | eo=この◆から**出る**側＝AEのイーズアウト＝**加速** ／ ei=この◆へ**入る**側＝AEのイーズイン＝**減速**。0〜100 |
  | 2 | 補間は `bezierEaseT` | **composer.html からそのまま転載**。区間 k→k+1 は `cubic-bezier(eo/100, 0, 1-ei/100, 1)` |
  | 3 | ◆を**クリック**でそのキーの緩急メニュー | `camKeyUp` で `!moved` なら `openKeyEaseMenu`。**ドラッグ（尺の配分）とは離した時点で切り分ける**ので、同じ◆に2つの役目を持たせられる |
  | 4 | 先頭の◆は「出」だけ / 最終の◆は「入」だけ | 意味の無い側を出さない（先頭に入る区間は無い） |
  | 5 | ◆の**形**で分かる（COMPOSER 合わせ） | 菱形=緩急なし / 角丸=中 / 丸=最大 / 灰の四角=タメ。色も `#5AE9FF` / `#FF4FA8` / `#8892a0` で COMPOSER と同じ。**econte は先頭/最終キーがもともと ice なので、効きのあるキーには外側の光る輪を足して区別**している |
  | 6 | 区間バーは残した | 現場の名前（等速/なめらか/加速/減速/クイックPAN/フェアリング/タメ）を**左右キーの eo/ei に落とすだけ**の早撃ち入口。プリセットに合わない組は `出 66% ／ 入 100%` と数字で出す（**嘘の名前を付けない**） |
  | 7 | 移行 | `migrateEase()` が旧名前をキーへ開く。**名前の逆転もここで直る**（旧 IN→eo / 旧 OUT→右キーの ei）。共有されるキーは**強いほうを残す** |

  **実測**: 加速（出80）= 25%地点で 0.036 / 75% で 0.422、
  減速（入80）= 25% で 0.578 / 75% で 0.964。タメは区間中 0 のまま。

- **§5-C10 読み戻し／リファレンス取り込み: 実装済み（2026-08-19）**

  **新しい層は1つも足していない。** econte では **BOARD の写真がそのまま参照**で、CUT枠で切り出せば
  `baseC`（＝REF。濃度・上下を切り替えられる下敷き）になる。だから C10 は
  **「写真を置いて、16:9 の枠を自動で合わせる」だけ**で足りる。

  | | 仕組み | 要点 |
  |---|---|---|
  | 1 | `+ カット` ボタン ／ **SHEET へ D&D** | **落とす場所で意味が決まる**: BOARD＝今までどおり写真（紙のネーム）／SHEET＝カットとして取り込む。狭い画面（≤880px）はラベルを `+◫` に畳む |
  | 2 | 1枚 = 1カット（16:9） | 動画から抜いたスクショを並べる用途がこれ。順番はファイルの順 |
  | 3 | **書き出したカラースクリプトPNGは自動で格子に分ける** | `csGeometry()` が **書き出しの寸法式を逆に解く**（W から列数と列間、H から行数・色帯・行間）。整数解が出た候補を**実際の画素**（余白・列間が真っ黒か）で絞る |
  | 4 | 最終行の**空き升目は落とす** | 8列で5コマ、のとき残り3つは真っ黒。**キャプション（C1-A のピンク文字）が下に有るか**で判定する（絵は真っ黒になり得るが、キャプションは必ず描かれる） |
  | 5 | 枠のほうを固定して画像を**収める** | ★ 逆（画像の幅を固定して枠を広げる）にすると、縦長の画像で枠が横へ伸びて **隣に置いた参照画像まで焼き込む**（2026-08-19 に踏んだ）。升目 1400×787.5 に contain で入れる |
  | 6 | 16:9 以外は余白＋⚠ | 端を切らない印として正しい。4:3 で uncov 25% / 縦位置で 67%（実測） |
  | 7 | `baseAlpha = 1` | 取り込んだ絵は**それ自体が画**なので薄くしない（トレースしたくなったら REF のスライダーで落とす）。BOARD から切り出した通常のカットは従来どおり 0.5 |
  | 8 | **取り込み全体で Undo 1手** | `gLogMute` を立てて個別の `photo-add` / `cut-add` を黙らせ、まとめて `cut-import` を1つ積む（8コマ分けたら16手、では使えない） |

  **実測**: 書き出した C.SCRIPT PNG を読み戻したセルは、書き出す前の `drawCellFrame` と
  **画素差 0%**（4列1行・列間あり・色帯あり・行間ありで確認）。
  誤判定の確認も済み——同寸法の白画像／ふつうの写真／黒いが寸法違いの画像は、いずれも格子扱いしない。

  **入れていないもの**: **動画の読み込み（→連番写真）**。ユーザーの判断で見送り
  （「ひとまず動画は自分で必要なとこをスクショして入れればよさそう」）。
  外でコマを抜いて `+ カット` に渡せば結果は同じになる（`crv` でも可）。
  将来やるなら **動画→連番写真→この経路に流す**だけで済むように、
  `importAsCuts()` は「画像の配列」を受け取る形にしてある。

- **§5-E 手つきの整理: 実装済み（2026-08-24）**

  実際に触って出てきた引っかかりを一気に片づけた回。**新しい層も新しいデータも足していない**——
  すべて「どの手つきで何が起きるか」の付け替え。

  | | 変えたこと | 要点 |
  |---|---|---|
  | 1 | **書き出しは「もう一度押す＝中断」** | 旧: WebCodecs 経路は押せるのに `exportVideo` が黙って `return`／実時間録画は `disabled` ＝ **どちらも止める手が無かった**。ボタンの click を `gExporting ? cancelExport() : exportVideo()` にして、表示を `■ MP4 42%` に。後始末は `resetExportBtn()` 1本に寄せた。**⇩ C.SCRIPT も同じ手つき**（`gCsBusy`。行ごとに `setTimeout(0)` を挟んで押せる隙を作る） |
  | 2 | ボタン表記 | `+ IMPORT` → **`▶ IMPORT`**（BOARDに写真を置く）／ `+ カット` → **`▶ C.SCRIPT`**（1枚=1カット）。**別ボタンのまま**にして、tooltip に「IMPORT との違い」を書いた。出す側の `⇩ C.SCRIPT` と ▶/⇩ で対になる |
  | 3 | BOARD の枠色を入れ替え | **選択中のCUT枠＝水色 `rgba(150,248,255,1)`／写真の選択枠＝グレー `rgba(205,205,215,.95)`**（SPEC_16 §5-C9 の「選択枠はグレー」は撤回）。現在カット（`foc`）は薄い水色 `.78` ＋ 細い線で、選択とは太さと角ハンドルで分ける |
  | 4 | **写真は「一度選んでから」でないと動かない** | `bDown` の `guard` を `!!state.selCut && !!p` から **`!!p && (!!state.selCut \|\| p.id !== state.selPhoto)`** へ。枠を選んでいない状態でも下の写真を掴んでズラす事故が残っていた。一触り目＝選択だけ、二触り目＝移動 |
  | 5 | 写真の DEL を撤去 | 🔗（REF）の隣で押し間違えて画像を消していたため。削除は **Delete キー**に一本化。★ただし **指の端末には Delete が無い**ので `@media (pointer: coarse)` のときだけ 🗑 を出す（`.touch-only`。REF の一群とは区切りを挟む） |
  | 6 | REF 解除のアイコン | ✕ は「画像を消す」と読み違えるので、**🔗 に赤い斜線**（`.btn.unlink::after`。`#btn-cut-vis` の斜線と同じ作り） |
  | 7 | REF の3つを一群に | 🔗 / ⇄ / 🔗̸ を `.refgrp`（破線の囲み＋小さな `REF` ラベル・font-size 8px）でまとめた。**隣の GRAY は「写真そのものを変える」／REF群は「ボードの元ネタと結ぶ」** で用途が違う、が色と大きさで分かる |
  | 8 | **L ＝ SHEETのロック**（Ctrl+L で全解除） | LOOP は常時ONで足りるので `def:''` にして**キーを外した**。`lockTargets()` は SHEET の複数選択 → 選択中の枠 → 現在カット の順に見る＝**手が触っているものが必ず対象**。まとめて触るときは「1つでも解いてあれば全部ロック」。旧 keymap を引きずらないよう LS キーを **`econte_keymap_v2`** に上げた |
  | 9 | 投げ縄ボタンの点灯を ANIMATOR と同じに | `lasso-on` = 紫 `#B98CFF` の6px丸（左上）／`fe-on` = 水色 `#5AE9FF` の6px丸（右上）。旧の「acidの四角＋金の破線outline」を差し替え。`pp-on`（筆圧）も金の丸に揃えた。ラベル `LASSO ⇄ L-DEL` は元から同じ |
  | 10 | パレットのスクロールを廃止 | `PAL_ROWS` / `fitPaletteRows` / `measurePalettes` を**削除**（§5-B5 の「縦3行＋スクロール」は撤回）。＋で足した色が畳まれた先に隠れて「増やしたのに出てこない」と読めてしまうため。8列のまま下へ伸ばす |
  | 11 | TIMELINE の横移動に**ホイール押しドラッグ** | `gTlPan`。strip の pointerdown で `e.button===1` を**スクラブより先に**見る（中ボタンでプレイヘッドを飛ばさない）。拡大していないときは「Alt+ホイールで拡大してから」と言う |
  | 12 | ◆の緩急は **Wクリック**（COMPOSER 合わせ） | 単クリック＝そのキーへ頭出し／**Wクリック・Ctrl+クリック＝緩急メニュー**。★`camKeyDown` が pointerdown で `preventDefault` するので**ネイティブ dblclick は来ない**＝`gKeyTap` に時刻と位置を持って自前で2回目を判定する（`DTAP_MS`/`DTAP_PX` を流用。指・ペン・マウスすべて同じ経路） |
  | 13 | ミニツールの HIDE をアイコンへ | `✕`（`.mini-x`）を **PEN の手前**に置いた。旧・右端の「HIDE」は行の長さで位置が動き、押す場所を毎回探すことになっていた |
  | 14 | **BOARD でも Alt+クリック＝スポイト** | Alt+ドラッグ＝パンはそのまま。`bDrag.eye` を持って **`bUp` で 5px 未満なら拾う**（◆の「動かさずに離したか」と同じ切り分け）。MOVE でも CUT でも効く |
  | 15 | **画面全体スポイト（ブラウザの外も可）** | `window.EyeDropper`（Chrome / Edge）。**現在色スウォッチのクリック**に割り当てた（`#color-swatch` / `#tl-mini-swatch`）。★ユーザー操作から**直接**呼ぶ必要がある（`setTimeout` 越しだと弾かれる）。Safari / iPad には無いので、無ければその旨を出す |
  | 16 | **SEL のコピー／ペースト**（別カットへ運べる） | `gSelClip`。COPY は**浮いたまま持ち出さず**、`flPaint` で「いま見えている形（拡縮・回転・WARP込み）」を1枚の紙に焼いてから控える。PASTE は貼り先の紙の**中央**に置いて、`copy:true` の `gFloat` を立てる（抜き跡を作らない）。カットごとにベイク寸法が違っても、紙より大きいときだけ縮める。**Undo は持ち上げ〜確定で1手**のまま |

  **実測（この回で確かめたこと）**
  - 書き出し中断: `■ MP4 56%` で押し直し → ラベルが戻り **blob は 1つも作られない**。⇩ C.SCRIPT も `■ C.SCRIPT 41%` → 中断でダウンロード0件、その直後に押せばふつうに出る
  - 枠色: CUT選択枠の実画素 `[150,248,255]` / 写真選択枠の実画素 `[205,205,215]`
  - 写真の掴み: 一触り目 = 選択のみ・移動量0／二触り目 = 移動する
  - 投げ縄バッジ: `rgb(185,140,255)` 6px 円（左上）／`rgb(90,233,255)` 6px 円（右上）＝ ANIMATOR と同値
  - パレット: 48セル・`overflow-y:visible`・`scrollHeight === clientHeight`（＝スクロールしない）
  - TIMELINE: 中ボタンドラッグで `tlView.start` 27→52・**プレイヘッドは動かない**
  - ◆: 1回目=メニュー出ない/頭出しのみ、2回目(W)=出る、Ctrl+クリック=出る
  - SEL: C1 で REF から持ち上げ → Ctrl+C → C2 へ移って Ctrl+V → Enter で **67600px（260×260）が C2 の line に焼ける**／Ctrl+Z 1手で 0 に戻る
  - L/Ctrl+L: L でフォーカスカットが lock、もう一度で解除、Ctrl+L で全解除。**LOOP は変わらない**
  - `.touch-only`: `(pointer: coarse)` で 🗑 が出る／マウス環境では `display:none`

  **未確認**: iPad 実機（`(pointer: coarse)` はエミュレーションで確認）。
  EyeDropper API は `window.EyeDropper` の存在まで（実際のピックはOSのピッカーが要る）。

- **§5-F 追い調整（2026-08-24・§5-E の当日）**

  | | 変えたこと | 要点 |
  |---|---|---|
  | 1 | **◆のWクリックが効かない本体を潰した** | ★真犯人は Wクリック判定ではなく **FIXカットの飾り◆の当たり判定**。T.U./PAN の **B（カット尻）と次のカットの FIX◆ は 4px しか離れず**（尻を1コマ内側に置く規則があっても、拡大していないと1コマ＝数px）、後から積まれる FIX 側が上に乗って **B のクリックを全部食っていた**。`.tl-key.fix{pointer-events:none}` で解決（飾りなので当たり判定は要らない）。title は `#tl-keys` の帯ごとに移した |
  | 2 | ◆のWクリック窓を広げた | `DTAP_MS(550)/DTAP_PX(48)` の流用をやめて **`KEY_DTAP_MS = 900` / `KEY_DTAP_PX = 40`**。◆の単クリックは「そのキーへ頭出し」＝2回押しても無害なので、取りこぼすより広く取るほうが良い（ペンで「コッ、コッ」と押すと 550ms は普通に超える） |
  | 3 | **GRID の Ctrl+A / Ctrl+D** | 全選択 / 解除。`colorCells()` の全キーを `setSelCells` に渡すだけ。`inGrid()` のときだけ見る |
  | 4 | ★ペイント系キーを**修飾キー付きで撃たないように** | `if(e.ctrlKey \|\| e.metaKey \|\| e.altKey) return;` を b/e/g/a/`[`/`]` の手前に置いた。**Ctrl+A が黙って SEL に切り替わっていた**（Ctrl+B/E/G も同様）。GRID の Ctrl+A を足して露見した積み残し |
  | 5 | **「余」を SETTINGS から BOARD の枠バーへ移した** | ★効くのは BOARD の焼く範囲なのに、**操作は BOARD を覆うモーダルの中**にあった＝動かしても変化が1つも見えなかった（ユーザーの「どこを見れば変化わかりますか？」がこれ）。置き場所は `#cutsel-box`（＋枠・∿ の隣）＝枠を選んだときにだけ出る |
  | 6 | 余は**枠が1つのカットではグレー** | `calcBakeRect` が `cam.length <= 1` で `e=1` に落とす＝FIXでは**元から効かない**。値は `—` にして、title に「＋枠 で T.U./PAN にすると触れます」と条件を書く |
  | 7 | **「焼く範囲」を BOARD に描く** | `bakeRect` を**紫の破線＋「焼く範囲 1.30」ラベル**で出す（枠2つ以上を選択中のときだけ）。旧は `uncov > 0` の赤ハッチのときしか描かれず、余を動かしても普通は何も変わらなく見えた |
  | 8 | 余はドラッグ中も追従する | `input` で `bakeRect` だけ仮計算して `renderBoard()`（**焼き直しは重いので `change` まで待つ**。`bakeExpand` は仮に入れてすぐ戻す）。`touch-action:none` を付けて、枠バーの横スクロールに指を取られないようにした |

  **実測**
  - `document.elementFromPoint(Bの中心)` = 修正前 `tl-key fix` → 修正後 **`tl-key edge`（k=B）**。B の Wクリックでメニューが開くことを実クリックで確認（`C1-B ◆ の緩急`）
  - Wクリック窓: 700ms 間隔=開く / 1200ms=開かない（頭出しのみ）/ 30px ずれても開く
  - Ctrl+A → 5コマ選択・HUD `5枚選択` ／ Ctrl+D → 0コマ。SINGLE では効かない
  - 余 1.02→1.30 で `bakeRect` 816×459 → **1040×585**、ベイク寸法の表示も 1813×1020 → 2311×1300
  - FIXカット選択時: 余がグレー・値 `—`・title に条件

- **§5-G 操作文法を ANIMATOR に揃えた（SPEC_17・2026-08-24）**

  「道具の切替をボタン探しから手癖へ」。**正準は `animator.html`**、econte はそこへ寄せた側。
  データ・保存形式は触っていない。

  | | 入れたもの | 要点 |
  |---|---|---|
  | 1 | **キーの押し方4通り＋バネ** | `PAINT_KEYS`（キー→ツールid）と `KEY_GESTURES`（id→押し方）の2枚。単押し=選択／2連打(320ms)=ボタンのWクリック／長押し(500ms)=ボタンの長押し／`Shift+`=長押しへ直行／**押しっぱなし→離す(250ms〜)=押している間だけそのツール**（離すと元へ戻る） |
  | 2 | ★**`G` の2連打には割り当てない** | econte は FILL廃止で常時投げ縄（§5-B2）＝ANIMATOR の「2連打＝投げ縄塗り⇔バケツ」に**当たる相手が居ない**。空振りする押し方を作らない。投げ縄消しは **G長押し / Shift+G**（ANIMATOR と同じ場所） |
  | 3 | `E` の2連打は登録してよい | ANIMATOR が erase に `dbl` を持たないのは、あちらのボタンが click の時刻差で全消去を見ていて二重発火するから。econte は `onDoubleActivate`＝ネイティブ dblclick なのでキーからは発火しない |
  | 4 | `P` を PEN の別名に | ANIMATOR は `P`、econte は昔から `B`。**両方効かせる**（片方を取り上げたら「手が覚え直し」という目的に反する）。`P` は econte の他のショートカットと衝突なし |
  | 5 | ★ボタンとキーは**同じ関数**を呼ぶ | `wire()` の中に閉じていた無名ハンドラを `togglePressurePen` / `eraseClearHere` / `applyFillModeUI` / `fillToggleLassoErase` としてトップレベルへ出した。挙動を二本立てにしない |
  | 6 | **多角形の投げ縄** | **ドラッグで始める＝フリーハンド／クリックで始める＝多角形**。確定は Enter / 始点クリック / Wクリック、`Esc` で捨てる（塗らない・Undoも積まない・ツールを変えても捨てる）。★**Alt+クリックは使わない**——スポイトの正準割り当てと衝突するし、「クリックで始めたら多角形」なら修飾キーが1つも要らない |
  | 7 | `stageToast()` | ステージ上部中央に0.9秒。**econte はステージが2つ**（STUDIOプレビュー / EDIT）あるので id ではなく `.stage-toast` を両方に置き、出ているほうへ出す。見えていなければ既存の `toast()` へ落とす |

  **移植で効いた ANIMATOR の落とし穴（そのまま踏襲）**
  - **`e.repeat` を捨てる**——押しっぱなしを連打と数えると、E の全消去が連射されて Undo が何手も積まれる
  - **単押しは遅らせない**（1回目で即ツール選択 → 2回目で切替）。「2連打か待つ」設計はツール選択がもたつく
  - **2連打・長押しが成立したら基準時刻を0に落とす**（3連打が2連打の続きにならないように）
  - `keyup` と `blur` でタイマーとバネ状態を必ず消す

  **econte 固有で気をつけた所**
  - `onKey` の修飾キー除けは **Shift だけ通す**（`Shift+G` が死ぬので）。`[`／`]` は Shift 無しのときだけ
  - 多角形の頂点打ちは `paintDown` の **`gFloat` より手前**で見る（浮いた選択の当たり判定に食われるため）
  - **別の面（GRIDの別セル等）を触ったら捨てる**（`gLasso.cv` と比較）
  - `#edit-stage` に `position:relative` が無かったので足した（`.stage-toast` の錨）
  - 始点に吸い付いて閉じる判定は **画面px**（`LASSO_CLOSE_PX=14`）。ズームしても指の感覚が変わらない

  **実測**
  - `G`単押し=LASSO選択 ／ `G`2連打=**何も起きない**（意図どおり・トーストも出ない） ／ `Shift+G`=`fillErase` 反転・ラベル `L-DEL`・stageToast「投げ縄 透明消し」 ／ `G`長押し620ms=同じ
  - `B`/`P` 2連打=筆圧ON/OFF（`pp-on` バッジも点く） ／ `E`2連打=線が 33928px → **0**、Ctrl+Z で戻る ／ `A`2連打=`WARP` ラベルに
  - `e.repeat` を挟んだ連打は**発火しない**
  - バネ: PEN中に `E` を320ms押して離す → **PEN へ戻る**（「PEN へ戻りました」）。80msだけ押した場合は切り替わったまま。`blur` を挟むと戻らない
...
- **持ち越し**: ブラシチップ（内蔵数種＋透過PNG読み込み。**.abrパーサは作らない**）。
  「いつも使うのは数種なのでこのブラシみたいにしたい」の具体指定が**後から来る**ので、先回りで作り込まない

...
- **V3-P2「浮いた選択」＝軽いフォトバッシュ: 実装済み（2026-08-19・SPEC_15 P2）**
  ...
  **LRU も保存フォーマットも一切触っていない**（旧 V2-E3 の `cut.layers[]` を採らなかった理由がここ）。
  ...
  | 4 | **★焼き込み先は「持ち上げた層」**（line→line / color→color・BASE→line） | SPEC_15 P2-4 は「色も line へ焼く」だったが、それは plate がカット1枚だった V3 の話。V4 で plate も枠ごとのパッチになった以上、color を line に落とすと**色帯・C.SCRIPT から絵が消える**。**切り替えは `floatKindOf()` の1行** |
  ...
  **失っているもの**: 回転（v1では入れない・要望が出たら枠の上に専用ハンドル）／
  確定後の変形やり直し（Ctrl+Z で戻してやり直す）。

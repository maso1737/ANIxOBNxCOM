...
| **econte.html** | 紙ネーム/ラフの写真(D&D・IMPORT・Ctrl+Vペースト・HEIC) / パレットJSON / 音(mp3·wav·m4a·aac) / **プロジェクトZIP `ECONTE_PROJECT_v1`（⇧ PROJ。2026-08-15〜 音とマーカーも同梱＝これ1つで丸ごと戻せる）** | **動画コンテ WebM/mp4（実時間録画 or WebCodecs非実時間・音入り・C#/尺/**TIME**焼き込み可）** / **カラースクリプト一覧PNG（単位は「カット」ではなく**カメラ枠**。`C1-A`/`C1-L` が各1セル・`cam[].key`＝SHEET行の★ で間引き。SPEC_13 §5h）** / **プロジェクトZIP `ECONTE_PROJECT_v1`（⇩ PROJ。写真＋枠列＋加筆＋色＋音＋マーカー。ベイクは含めない。`ver:3` から**色は `cuts/<id>.plate.png` のカット1枚**＝ベイク空間×0.4・長辺1024上限。濃/描画モードは `meta.paint` の全体設定1組。`ver:2` 以前の枠ごと `colors[]` は読み込み時に1枚へ変換する。SPEC_15 P1）** / **P2予定: animator REF(`tdr_live`)** | なし（プリプロに専念） |

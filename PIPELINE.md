...
| **econte.html** | 紙ネーム/ラフの写真(D&D・IMPORT・Ctrl+Vペースト・HEIC) / パレットJSON / 音(mp3·wav·m4a·aac) / **プロジェクトZIP `ECONTE_PROJECT_v1`（⇧ PROJ。2026-08-15〜 音とマーカーも同梱＝これ1つで丸ごと戻せる）** | **動画コンテ WebM/mp4（実時間録画 or WebCodecs非実時間・音入り・C#/尺/**TIME**焼き込み可）** / **カラースクリプト一覧PNG（単位は「カット」ではなく**カメラ枠**。`C1-A`/`C1-L` が各1セル・`cam[].key`＝SHEET行の★ で間引き。SPEC_13 §5h）** / **プロジェクトZIP `ECONTE_PROJECT_v1`（⇩ PROJ。写真＋枠列＋加筆＋色＋音＋マーカー。ベイクは含めない。`ver:4` から**画は枠ごと**＝`cuts/<id>.f<k>.line.png`（1280×720・出力等倍）＋ `cuts/<id>.f<k>.plate.png`（512×288）が `cam[k]` と1対1。濃/描画モードは `meta.paint` の全体設定1組。`ver:3`（カット1枚・ベイク空間）は枠の矩形で切り出して配り、`ver:2` 以前の枠ごと `colors[]` はそのまま plate パッチへ1対1で載る。SPEC_16）** / **P2予定: animator REF(`tdr_live`)** | なし（プリプロに専念） |

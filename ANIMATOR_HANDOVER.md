...
**次にやる（仕様確定済み・着手待ち）**
- **作画タイムラプス P1（アプリ内再生）** → **`SPEC_14_TIMELAPSE.md`**。
  **P0（記録エンジン）は 2026-08-01 実装済み**＝もう撮れている。残りは P1（TIMELAPSEパネル・
  プレビュー・トランスポート・**WORK/CELL 並べ替え**・情報表示・設定UI・CLEAR）と P2（WebM/MP4書き出し）。
...
- ショットは `frameIdx` を持つので、P1 で **WORK（作業順）/ CELL（コマ別）** の2通りに並べ替えられる
- 別窓（`?ro=1`）は `TL －` で記録しない。再生UI・動画書き出しは **P1/P2 で未実装**
